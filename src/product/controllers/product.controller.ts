import { Body, Controller, Get, HttpStatus, Param, Post, Put, Query, Req, Res, UseGuards, Delete } from '@nestjs/common';
...
export class ProductController {
    ...
    @ApiOperation({summary:'Obtain paginated product list included referenced owner'})
    @UseGuards(AuthGuard())         
    ...
    @UseGuards(AuthGuard()) 
    async updateProduct(@Res() res:Response,@Param('id') id:string, @Body() updateProductDto: UpdateProductDto, @Req() req: Request): Promise<any>{      
      const {user} = <any>req;
      const ownerId = String(user._id);
      let product = await this.productService.updateProduct(updateProductDto,id,ownerId);       
      if(product == 0){
        return res.status(HttpStatus.UNAUTHORIZED).json({message:`this is not the product's owner`});       
      }
      if(product == 1){
        return res.status(HttpStatus.NOT_FOUND).json({message:'There is no data'});       
      }
    ...
    @UseGuards(AuthGuard()) 
    async removeProduct(@Res() res:Response,@Param('id') id:string, @Req() req: Request): Promise<any>{      
      const {user} = <any>req;
      const ownerId = String(user._id);
      let product = await this.productService.removeProduct(id, ownerId);       
      if(product == 0){
        return res.status(HttpStatus.UNAUTHORIZED).json({message:`this is not the product's owner`});       
      }
      if(product == 1){
